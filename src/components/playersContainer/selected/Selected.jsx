...
const Selected = ({ selectedPlayers,setSelectedPlayers,coins,setCoins,setContainerState}) => {
...
{selectedPlayers.map((player)=><CardSelected  key={player.id} player={player} setSelectedPlayers={setSelectedPlayers} selectedPlayers={selectedPlayers} coins={coins} setCoins={setCoins}/>)}
  ...
  <div><button onClick={()=>setContainerState('available')} className='btn bg-linear-[90deg,#E05C51,#FE9F30] border-none rounded-lg '>Add player</button></div>
  </section>
};
